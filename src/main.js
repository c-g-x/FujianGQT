...
import gm from 'gm';
import {getMembers, getSecretaries} from './utils/config-loading.js'
...
async function autoLearning(member, tryCount) {
    ...
    axios.post(url, qs.stringify(data), {headers: {'Cookie': cookies}}).then((resp) => { // 做大学习
...
function getIncompleteMembers(secretary) {
    getCookiesAndValidationCode().then(object => {
        ...
        axios.post(url, qs.stringify(data), {headers: {'Cookie': object.cookies}}).then((resp) => { // 做大学习
            if (resp?.data?.success !== true) {
            ...
            setTimeout(() => {
                /* 获取当前最新的季度号 */
                axios.post('https://m.fjcyl.com/admin/cylOrgMembers/groupByList?groupBy=1', null, {headers: {'Cookie': object.cookies}}).then(async resp => {
                    ...
                    // 获取最新一期的 group id
                    axios.post(`https://m.fjcyl.com/admin/cylOrgMembers/selectList?orderBy=1&quarterNo=${quarterNo}`, null, {headers: {'Cookie': object.cookies}})
                        .then(resp => {
                            ...
                            if (groupStudyId) {
                                ...
                                axios.post(`https://m.fjcyl.com/admin/cylOrgMembers/selectCurrentStudy?` +
                                    ...
                                    null, {headers: {'Cookie': object.cookies}}).then((resp) => {
